package com.tomsom999.testmod.world.feature.tree;
...
{
	public static final IBlockState LOG = ModBlocks.log.getDefaultState().withProperty(CustomBlockLog.VARIANT, EnumHandeler.EnumType.SILVER);
	public static final IBlockState LEAF = ModBlocks.leaves.getDefaultState().withProperty(CustomBlockLeaves.VARIANT, EnumHandeler.EnumType.SILVER);
	
	private int minHeight;
	
	public WorldGenSilverTree() 
	{
		super(false);
		this.minHeight = 7;
	}
	...
	public boolean generate(World world, Random rand, BlockPos pos) 
	{
		...
		else
		{
			...
			if(isSoil && y < world.getHeight() - height - 1)
			{
				...
				for(int yPos = y - 4 + height; yPos <= y + height; yPos++)
				{
				...
				for(int logHeight = 0; logHeight < height; logHeight++)
				{
					...
					if(logState.getBlock().isAir(logState, world, up) || logState.getBlock().isLeaves(logState, world, up))
					{
						...
						{
							this.setBlockAndNotifyAdequately(world, pos.add(0, i, 0), LOG);
